...
const submit = () => {
    form.post(route('manager.meeting-memos.review', props.memo.id), {
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('manager.meeting-memos.index')" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
        ...
        <div class="py-8">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                <!-- Memo Detail -->
                 <div class="flex items-center gap-3">
                    <a v-if="memo.status === 'approved'" :href="route('manager.meeting-memos.pdf', memo.id)" class="inline-flex items-center gap-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <p class="text-sm dark:text-gray-500">Submitted by <span class="font-medium dark:text-gray-100">{{ memo.creator?.name }}</span></p>
                            <p class="text-sm dark:text-gray-500"> date: <span class="font-medium dark:text-gray-100">{{ memo.formatted_meeting_date }}</span></p>
                        </div>
                    ...
                    <div class="prose max-w-none">
                        <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 text-gray-800 dark:text-gray-100 text-sm leading-relaxed" v-html="memo.content"></div>
                    </div>
                </div>
                ...
                <div v-if="memo.attachments && memo.attachments.length > 0" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div v-for="attachment in memo.attachments" :key="attachment.id" class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-200 transition-colors">
                ...
                <!-- Review Form (only if pending_manager) -->
                <div v-if="memo.status === 'pending_manager'" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
