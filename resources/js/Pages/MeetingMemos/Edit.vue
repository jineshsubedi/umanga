...
const props = defineProps({ memo: Object, users: Array, company: Object });
...
const deleteExistingAttachment = (attachment) => {
    if (confirm(`Remove ${attachment.file_name}?`)) {
        router.delete(route('memos.attachments.destroy', attachment.id), {
            preserveScroll: true,
        });
    }
};
...
const submit = () => {
    ...
    form.post(route('memos.update', props.memo.id), {
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
            form.attachments = [];
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('memos.index')" class="text-gray-400 dark:text-gray-400 hover:text-gray-600 dark:hover:text-gray-600">
        ...
        <div class="py-8">
            <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div v-if="memo.status === 'rejected' && memo.latest_review?.comment"
                    ...
                    <p class="text-sm font-semibold text-red-700">Rejected — Reviewer's feedback:</p>
