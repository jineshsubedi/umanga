...
const submit = () => router.post(route('staff.meeting-memos.submit', props.memo.id));
const duplicate = () => {
    if (confirm('Create a new draft based on this rejected memo?')) {
        router.post(route('staff.meeting-memos.duplicate', props.memo.id));
    }
};
</script>
    ...
    <AuthenticatedLayout>
        ...
        <div class="py-8">
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                <!-- Actions -->
                <!-- Actions -->
                <div v-if="memo.status === 'draft'" class="flex gap-3">
                    <Link :href="route('staff.meeting-memos.edit', memo.id)"
                ...
                <div v-if="memo.status === 'rejected'" class="flex gap-3">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                    <div class="flex items-center gap-6 mb-4 pb-4 border-b border-gray-100">
                        ...
                        <div v-if="memo.managers && memo.managers.length > 0" class="border-l border-gray-100 pl-6">
                            <p class="text-sm text-gray-500">Assigned Managers</p>
                            <div class="flex flex-wrap gap-1.5 mt-1">
                                <span v-for="m in memo.managers" :key="m.id" class="px-2.5 py-0.5 bg-indigo-50 text-indigo-700 rounded-full text-xs font-medium border border-indigo-100">
                                    {{ m.name }}
                                </span>
                            </div>
                        </div>
                ...
                <div v-if="memo.attachments && memo.attachments.length > 0" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div v-for="attachment in memo.attachments" :key="attachment.id" class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-200 transition-colors">
