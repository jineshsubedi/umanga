...
watch([search, company_id, staff_id, approver_id], ([newSearch, newCompany, newStaff, newApprover]) => {
    router.get(route('super-admin.procurements.index'), {
        status: props.status,
        search: newSearch,
...
<template>
    <Head title="All Company Procurements" />
    <AuthenticatedLayout>
        <!-- Hero Strip -->
        <div class="bg-gradient-to-br from-[#3b0e77] via-[#5b14b8] to-[#2c0b5c] pt-8 pb-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
            ...
            <div class="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-3xl font-extrabold text-white tracking-tight">All Company Procurements</h1>
                        <p class="mt-1 text-purple-200 text-sm">Overview of meeting procurements across all companies.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-12 relative z-10 pb-12 space-y-6">
            ...
            <div class="flex gap-1 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl p-1.5 rounded-xl shadow-lg border border-white/30 dark:border-gray-700 w-fit">
                <Link v-for="tab in tabs" :key="tab"
                    :href="route('super-admin.procurements.index', { status: tab })"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-100 dark:divide-gray-700/50">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/50">
                            <tr v-for="procurement in procurements.data" :key="procurement.id" class="hover:bg-purple-50/50 dark:hover:bg-purple-900/10 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="font-bold text-gray-900 dark:text-white text-sm">{{ procurement.title }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                                        <div class="w-6 h-6 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-[10px] font-bold text-gray-600 dark:text-gray-300">
                                            {{ procurement.requester?.name?.charAt(0) }}
                                        </div>
                                        {{ procurement.requester?.name }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-600 dark:text-gray-400">{{ procurement.formatted_request_date }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold border" :class="statusBadge(procurement.status)">
                                        {{ procurement.status.replace('_', ' ') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <Link :href="route('super-admin.procurements.show', procurement.id)" class="inline-flex items-center px-3 py-1.5 text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/40 rounded-lg border border-blue-100 dark:border-blue-800 transition-colors">
                                        View
                                    </Link>
                                </td>
                            </tr>
                            <tr v-if="procurements.data.length === 0">
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center text-gray-400 dark:text-gray-500">
                                        ...
                                        <p class="font-bold text-sm">No {{ status === 'all' ? '' : status }} procurements found.</p>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <!-- Pagination -->
                    <div v-if="procurements.links && procurements.links.length > 3" class="px-6 py-4 bg-gray-50/50 dark:bg-gray-900/30 border-t border-gray-100 dark:border-gray-700/50 flex justify-center gap-1">
                        <template v-for="(link, p) in procurements.links" :key="p">
